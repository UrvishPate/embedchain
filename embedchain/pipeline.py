...
class Pipeline(EmbedChain):
    ...
    def __init__(self, id: str=None, name: str=None, config: PipelineConfig=None, db: BaseVectorDB=None, embedding_model: BaseEmbedder=None, llm: BaseLlm=None, yaml_path: str=None, log_level=logging.INFO, auto_deploy: bool=False):
        """
    ...
    def _init_client(self):
        ...
        if config.get('api_key'):
            ...
            api_key = input('🔑 Enter your Embedchain API key. You can find the API key at https://app.embedchain.ai/settings/keys/ \n')
    ...
    def _get_pipeline(self, id):
        ...
        r = requests.get(url, headers={'Authorization': f'Token {self.client.api_key}'})
    ...
    def _create_pipeline(self):
        ...
        payload = {'yaml_config': json.dumps(self.yaml_config), 'name': self.name, 'local_id': self.local_id}
        url = f'{self.client.host}/api/v1/pipelines/cli/create/'
        r = requests.post(url, json=payload, headers={'Authorization': f'Token {self.client.api_key}'})
        if r.status_code not in [200, 201]:
            raise Exception(f'❌ Error occurred while creating pipeline. API response: {r.text}')
        if r.status_code == 200:
            print(f"🎉🎉🎉 Existing pipeline found! View your pipeline: https://app.embedchain.ai/pipelines/{r.json()['id']}\n")
        elif r.status_code == 201:
            print(f"🎉🎉🎉 Pipeline created successfully! View your pipeline: https://app.embedchain.ai/pipelines/{r.json()['id']}\n")
        return r.json()

    def _get_presigned_url(self, data_type, data_value):
        payload = {'data_type': data_type, 'data_value': data_value}
        r = requests.post(f'{self.client.host}/api/v1/pipelines/{self.id}/cli/presigned_url/', json=payload, headers={'Authorization': f'Token {self.client.api_key}'})
        r.raise_for_status()
        return r.json()
    ...
    def search(self, query, num_documents=3):
        ...
        if self.id is None:
            ...
            context = self.db.query(query, n_results=num_documents, where=where, skip_embedding=False, citations=True)
            result = []
            for c in context:
                result.append({'context': c[0], 'source': c[1], 'document_id': c[2]})
            return result
        else:
    ...
    def _upload_data_to_pipeline(self, data_type, data_value, metadata=None):
        payload = {'data_type': data_type, 'data_value': data_value, 'metadata': metadata}
        try:
    ...
    def _mark_data_as_uploaded(self, data_hash):
        self.cursor.execute('UPDATE data_sources SET is_uploaded = 1 WHERE hash = ? AND pipeline_id = ?', (data_hash, self.local_id))
        self.connection.commit()
    ...
    def deploy(self):
        ...
        results = self.cursor.execute('SELECT * FROM data_sources WHERE pipeline_id = ? AND is_uploaded = 0', (self.local_id,)).fetchall()
        if len(results) > 0:
    ...
    def from_config(cls, yaml_path: str, auto_deploy: bool=False):
        ...
        embedding_model = EmbedderFactory.create(embedding_model_provider, embedding_model_config_data.get('config', {}))
        event_properties = {'init_type': 'yaml_config'}
        AnonymousTelemetry().capture(event_name='init', properties=event_properties)
        return cls(config=pipeline_config, llm=llm, db=db, embedding_model=embedding_model, yaml_path=yaml_path, auto_deploy=auto_deploy)
