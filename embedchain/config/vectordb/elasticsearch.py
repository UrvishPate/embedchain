...
class ElasticsearchDBConfig(BaseVectorDbConfig):
    ...
    def __init__(self, collection_name: Optional[str]=None, dir: Optional[str]=None, es_url: Union[str, List[str]]=None, **ES_EXTRA_PARAMS: Dict[str, any]):
        """
        ...
        if not self.ES_URL:
            raise AttributeError('Elasticsearch needs a URL attribute, this can either be passed to `ElasticsearchDBConfig` or as `ELASTICSEARCH_URL` in `.env`')
        self.ES_EXTRA_PARAMS = ES_EXTRA_PARAMS
        if not self.ES_EXTRA_PARAMS.get('api_key') and (not self.ES_EXTRA_PARAMS.get('basic_auth')) and (not self.ES_EXTRA_PARAMS.get('bearer_auth')) and (not self.ES_EXTRA_PARAMS.get('http_auth')):
