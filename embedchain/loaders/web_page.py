...
try:
    ...
    raise ImportError('Webpage requires extra dependencies. Install with `pip install --upgrade "embedchain[dataloaders]"`') from None
...
class WebPageLoader(BaseLoader):
    ...
    def _get_clean_content(self, html, url) -> str:
        ...
        if original_size != 0:
            logging.info(f'[{url}] Cleaned page size: {cleaned_size} characters, down from {original_size} (shrunk: {original_size - cleaned_size} chars, {round((1 - cleaned_size / original_size) * 100, 2)}%)')
        return content