...
class GmailLoader(BaseLoader):
    ...
    def load_data(self, query):
        ...
        if not os.path.isfile('credentials.json'):
            raise FileNotFoundError('You must download the valid credentials file from your google                 dev account. Refer this `https://cloud.google.com/docs/authentication/api-keys`')
        ...
        for document in documents:
            ...
            meta_data = {'url': document.metadata.get('id'), 'date': get_header(document.text, 'Date'), 'subject': get_header(document.text, 'Subject'), 'from': get_header(document.text, 'From'), 'to': get_header(document.text, 'To'), 'search_query': query}
            ...
            if original_size != 0:
                logging.info(f'[{id}] Cleaned page size: {cleaned_size} characters, down from {original_size} (shrunk: {original_size - cleaned_size} chars, {round((1 - cleaned_size / original_size) * 100, 2)}%)')
