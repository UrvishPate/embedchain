...
class JSONLoader(BaseLoader):
    ...
    def load_data(content):
        ...
        try:
            from llama_hub.jsondata.base import JSONDataReader as LLHBUBJSONLoader
        except ImportError:
            raise Exception(f"Couldn't import the required packages to load {content},                 Do `pip install --upgrade 'embedchain[json]`")
        ...
        if os.path.isfile(content):
            ...
            if response.status_code == 200:
                ...
                raise ValueError(f'Loading data from the given url: {content} failed.                     Make sure the url is working.')
        else:
