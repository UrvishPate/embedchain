    ...
    def __init__(self, api_key=None, host='https://apiv2.embedchain.ai'):
        ...
        if api_key:
            if self.check(api_key):
                ...
                raise ValueError('Invalid API key provided. You can find your API key on https://app.embedchain.ai/settings/keys.')
        elif 'api_key' in self.config_data:
            self.api_key = self.config_data['api_key']
            logging.info('API key loaded successfully!')
        else:
            raise ValueError('You are not logged in. Please obtain an API key from https://app.embedchain.ai/settings/keys/')
