...
try:
    ...
    raise ImportError("Weaviate requires extra dependencies. Install with `pip install --upgrade 'embedchain[weaviate]'`") from None
...
class WeaviateDB(BaseVectorDB):
    ...
    def __init__(self, config: Optional[WeaviateDBConfig]=None):
        """Weaviate as vector database.
        ...
        if config is None:
            ...
            if not isinstance(config, WeaviateDBConfig):
                raise TypeError('config is not a `WeaviateDBConfig` instance. Please make sure the type is right and that you are passing an instance.')
            self.config = config
        self.client = weaviate.Client(url=os.environ.get('WEAVIATE_ENDPOINT'), auth_client_secret=weaviate.AuthApiKey(api_key=os.environ.get('WEAVIATE_API_KEY')), **self.config.extra_params)
    ...
    def _initialize(self):
        ...
        if not self.client.schema.exists(self.index_name):
            ...
            class_obj = {'classes': [{'class': self.index_name, 'vectorizer': 'none', 'properties': [{'name': 'identifier', 'dataType': ['text']}, {'name': 'text', 'dataType': ['text']}, {'name': 'metadata', 'dataType': [self.index_name + '_metadata']}]}, {'class': self.index_name + '_metadata', 'vectorizer': 'none', 'properties': [{'name': 'data_type', 'dataType': ['text']}, {'name': 'doc_id', 'dataType': ['text']}, {'name': 'url', 'dataType': ['text']}, {'name': 'hash', 'dataType': ['text']}, {'name': 'app_id', 'dataType': ['text']}]}]}
    ...
    def get(self, ids: Optional[List[str]]=None, where: Optional[Dict[str, any]]=None, limit: Optional[int]=None):
        ...
        while cursor is not None or not has_iterated_once:
            ...
            results = self._query_with_cursor(self.client.query.get(self.index_name, ['identifier']).with_additional(['id']).with_limit(self.BATCH_SIZE), cursor)
    ...
    def add(self, embeddings: List[List[float]], documents: List[str], metadatas: List[object], ids: List[str], skip_embedding: bool):
        ...
        self.client.batch.configure(batch_size=self.BATCH_SIZE, timeout_retries=3)
        with self.client.batch as batch:
            for (id, text, metadata, embedding) in zip(ids, documents, metadatas, embeddings):
                ...
                obj_uuid = batch.add_data_object(data_object=copy.deepcopy(doc), class_name=self.index_name, vector=embedding)
                metadata_uuid = batch.add_data_object(data_object=copy.deepcopy(updated_metadata), class_name=self.index_name + '_metadata', vector=embedding)
        ...
        if len(keys.intersection(self.metadata_keys)) != 0:
            ...
            for key in keys:
                if key in self.metadata_keys:
                    weaviate_where_operands.append({'path': ['metadata', self.index_name + '_metadata', key], 'operator': 'Equal', 'valueText': where.get(key)})
            ...
            results = self.client.query.get(self.index_name, data_fields).with_where(weaviate_where_clause).with_near_vector({'vector': query_vector}).with_limit(n_results).do()
        else:
            results = self.client.query.get(self.index_name, data_fields).with_near_vector({'vector': query_vector}).with_limit(n_results).do()
    ...
    def reset(self):
        ...
        """
        self.client.batch.delete_objects(self.index_name, where={'path': ['identifier'], 'operator': 'Like', 'valueText': '.*'})
