...
class QdrantDB(BaseVectorDB):
    ...
    def __init__(self, config: QdrantDBConfig=None):
        ...
        if config is None:
            ...
            raise TypeError('config is not a `QdrantDBConfig` instance. Please make sure the type is right and that you are passing an instance.')
        self.config = config
    ...
    def _initialize(self):
        ...
        if self.collection_name not in collection_names:
            self.client.recreate_collection(collection_name=self.collection_name, vectors_config=VectorParams(size=self.embedder.vector_dimension, distance=Distance.COSINE, hnsw_config=self.config.hnsw_config, quantization_config=self.config.quantization_config, on_disk=self.config.on_disk))
    ...
    def get(self, ids: Optional[List[str]]=None, where: Optional[Dict[str, any]]=None, limit: Optional[int]=None):
        ...
        qdrant_must_filters = [models.FieldCondition(key='identifier', match=models.MatchAny(any=ids))]
        if len(keys.intersection(self.metadata_keys)) != 0:
            for key in keys.intersection(self.metadata_keys):
                qdrant_must_filters.append(models.FieldCondition(key='metadata.{}'.format(key), match=models.MatchValue(value=where.get(key))))
        offset = 0
        existing_ids = []
        while offset is not None:
            response = self.client.scroll(collection_name=self.collection_name, scroll_filter=models.Filter(must=qdrant_must_filters), offset=offset, limit=self.BATCH_SIZE)
            offset = response[1]
    ...
    def add(self, embeddings: List[List[float]], documents: List[str], metadatas: List[object], ids: List[str], skip_embedding: bool):
        ...
        for i in range(0, len(qdrant_ids), self.BATCH_SIZE):
            self.client.upsert(collection_name=self.collection_name, points=Batch(ids=qdrant_ids[i:i + self.BATCH_SIZE], payloads=payloads[i:i + self.BATCH_SIZE], vectors=embeddings[i:i + self.BATCH_SIZE]))
    ...
    def query(self, input_query: List[str], n_results: int, where: Dict[str, any], skip_embedding: bool, citations: bool=False) -> Union[List[Tuple[str, str, str]], List[str]]:
        ...
        if len(keys.intersection(self.metadata_keys)) != 0:
            for key in keys.intersection(self.metadata_keys):
                qdrant_must_filters.append(models.FieldCondition(key='payload.metadata.{}'.format(key), match=models.MatchValue(value=where.get(key))))
        results = self.client.search(collection_name=self.collection_name, query_filter=models.Filter(must=qdrant_must_filters), query_vector=query_vector, limit=n_results)
        contexts = []
