...
class ChromaDB(BaseVectorDB):
    ...
    def add(self, embeddings: List[List[float]], documents: List[str], metadatas: List[object], ids: List[str], skip_embedding: bool) -> Any:
        ...
        for i in range(0, len(documents), self.BATCH_SIZE):
            ...
            if skip_embedding:
                self.collection.add(embeddings=embeddings[i:i + self.BATCH_SIZE], documents=documents[i:i + self.BATCH_SIZE], metadatas=metadatas[i:i + self.BATCH_SIZE], ids=ids[i:i + self.BATCH_SIZE])
            else:
    ...
    def query(self, input_query: List[str], n_results: int, where: Dict[str, any], skip_embedding: bool, citations: bool=False) -> Union[List[Tuple[str, str, str]], List[str]]:
        ...
        try:
            if skip_embedding:
                result = self.collection.query(query_embeddings=[input_query], n_results=n_results, where=where)
            else:
                result = self.collection.query(query_texts=[input_query], n_results=n_results, where=where)
