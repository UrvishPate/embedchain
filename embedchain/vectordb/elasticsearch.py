...
try:
    ...
    raise ImportError('Elasticsearch requires extra dependencies. Install with `pip install --upgrade embedchain[elasticsearch]`') from None
...
class ElasticsearchDB(BaseVectorDB):
    ...
    def __init__(self, config: Optional[ElasticsearchDBConfig]=None, es_config: Optional[ElasticsearchDBConfig]=None):
        """Elasticsearch as vector database.
        ...
        if config is None and es_config is None:
            ...
            if not isinstance(config, ElasticsearchDBConfig):
                raise TypeError('config is not a `ElasticsearchDBConfig` instance. Please make sure the type is right and that you are passing an instance.')
    ...
    def _initialize(self):
        ...
        index_settings = {'mappings': {'properties': {'text': {'type': 'text'}, 'embeddings': {'type': 'dense_vector', 'index': False, 'dims': self.embedder.vector_dimension}}}}
    ...
    def query(self, input_query: List[str], n_results: int, where: Dict[str, any], skip_embedding: bool, citations: bool=False) -> Union[List[Tuple[str, str, str]], List[str]]:
        ...
        query = {'script_score': {'query': {'bool': {'must': [{'exists': {'field': 'text'}}]}}, 'script': {'source': "cosineSimilarity(params.input_query_vector, 'embeddings') + 1.0", 'params': {'input_query_vector': query_vector}}}}
        if 'app_id' in where:
            app_id = where['app_id']
