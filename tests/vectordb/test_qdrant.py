...
class TestQdrantDB(unittest.TestCase):
    ...
    def test_add(self, uuid_mock, qdrant_client_mock):
        ...
        qdrant_client_mock.return_value.upsert.assert_called_once_with(collection_name='embedchain-store-1526', points=Batch(ids=['abc', 'def'], payloads=[{'identifier': '123', 'text': 'This is a test document.', 'metadata': {'text': 'This is a test document.'}}, {'identifier': '456', 'text': 'This is another test document.', 'metadata': {'text': 'This is another test document.'}}], vectors=embeddings))
    ...
    def test_query(self, qdrant_client_mock):
        ...
        qdrant_client_mock.return_value.search.assert_called_once_with(collection_name='embedchain-store-1526', query_filter=models.Filter(must=[models.FieldCondition(key='payload.metadata.doc_id', match=models.MatchValue(value='123'))]), query_vector=['This is a test document.'], limit=1)
    ...
    def test_reset(self, qdrant_client_mock):
        ...
        App(config=app_config, db=db, embedder=embedder)
        db.reset()
        qdrant_client_mock.return_value.delete_collection.assert_called_once_with(collection_name='embedchain-store-1526')
