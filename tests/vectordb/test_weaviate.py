...
class TestWeaviateDb(unittest.TestCase):
    ...
    def test_add(self, weaviate_mock):
        ...
        weaviate_client_batch_enter_mock.add_data_object.assert_any_call(data_object={'text': documents[0]}, class_name='Embedchain_store_1526_metadata', vector=embeddings[0])
        weaviate_client_batch_enter_mock.add_data_object.assert_any_call(data_object={'text': documents[1]}, class_name='Embedchain_store_1526_metadata', vector=embeddings[1])
        weaviate_client_batch_enter_mock.add_data_object.assert_any_call(data_object={'identifier': ids[0], 'text': documents[0]}, class_name='Embedchain_store_1526', vector=embeddings[0])
        weaviate_client_batch_enter_mock.add_data_object.assert_any_call(data_object={'identifier': ids[1], 'text': documents[1]}, class_name='Embedchain_store_1526', vector=embeddings[1])
    ...
    def test_query_without_where(self, weaviate_mock):
        ...
        weaviate_client_query_get_mock.with_near_vector.assert_called_once_with({'vector': ['This is a test document.']})
    ...
    def test_query_with_where(self, weaviate_mock):
        ...
        weaviate_client_query_mock.get.assert_called_once_with('Embedchain_store_1526', ['text'])
        weaviate_client_query_get_mock.with_where.assert_called_once_with({'operator': 'Equal', 'path': ['metadata', 'Embedchain_store_1526_metadata', 'doc_id'], 'valueText': '123'})
        weaviate_client_query_get_where_mock.with_near_vector.assert_called_once_with({'vector': ['This is a test document.']})
    ...
    def test_reset(self, weaviate_mock):
        ...
        App(config=app_config, db=db, embedder=embedder)
        db.reset()
        weaviate_client_batch_mock.delete_objects.assert_called_once_with('Embedchain_store_1526', where={'path': ['identifier'], 'operator': 'Like', 'valueText': '.*'})
