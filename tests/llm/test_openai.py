...
def config():
    ...
    config = BaseLlmConfig(temperature=0.7, max_tokens=50, top_p=0.8, stream=False, system_prompt='System prompt', model='gpt-3.5-turbo')
    yield config
...
def test_get_llm_model_answer_without_system_prompt(config, mocker):
    ...
    llm = OpenAILlm(config)
    llm.get_llm_model_answer('Test query')
    mocked_jinachat.assert_called_once_with(model=config.model, temperature=config.temperature, max_tokens=config.max_tokens, model_kwargs={'top_p': config.top_p})